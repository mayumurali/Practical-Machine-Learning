...
```{r Removing NearZeroVariance variables}
...
```{r Removing first column of mytraining data}
myTraining <- myTraining[c(-1)]
```
Clean variables with more than 60% NA
```{r Cleaning variables with more than 60% NA}
...
```{r Predicting with Decision Trees}
set.seed(12345)
...
```{r Predicting results on test data}
